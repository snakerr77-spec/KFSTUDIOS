...
function buildWhatsAppMessage({ nome, whatsapp, tipo, mensagem }) {
  ...
  const tipoEnsaio = normalizeSpaces(tipo);
  const textoCliente = normalizeSpaces(mensagem);

  const lines = [
    'Olá, KFSTUDIOS! Vim pelo site e gostaria de fazer um orçamento de ensaio fotográfico.',
    '',
    `Meu nome é ${nomeCliente}.`
  ];

  if (tipoEnsaio) lines.push(`Tenho interesse em: ${tipoEnsaio}.`);
  if (telefoneCliente) lines.push(`Meu WhatsApp: ${telefoneCliente}.`);
  if (textoCliente) {
    lines.push('', `Mensagem: ${textoCliente}`);
  }

  lines.push('', 'Pode me passar os valores, datas disponíveis e como funciona o agendamento?');
  return lines.join('\n');
}

function buildQuickWhatsAppMessage(origin = 'site') {
  const place = origin === 'rodape' ? 'pelo rodapé do site' : 'pelo site';
  return [
    `Olá, KFSTUDIOS! Vim ${place} e gostaria de saber mais sobre os ensaios fotográficos.`,
    '',
    'Pode me passar os valores, datas disponíveis e como funciona o agendamento?'
  ].join('\n');
}
